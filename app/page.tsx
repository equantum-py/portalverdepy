    ...
    <>
      <main className="container-shell space-y-5 py-4 sm:space-y-6 sm:py-6 lg:py-10">
        {content.sections.filter(section=>section.isActive).sort((a,b)=>a.sortOrder-b.sortOrder).map(section=>{
          if(section.key==='hero') return <div key={section.key}><div className="space-y-4 lg:hidden"><CategorySidebar/><HomeHero/></div><div className="hidden gap-5 lg:grid lg:grid-cols-[280px_minmax(0,1fr)]"><CategorySidebar/><HomeHero/></div></div>;
