...
export function SectionBannerUploader({
  sectionKey,
  variant,
  url,
  onChange,
  ...
  const recommended = variant === 'desktop' ? '600 × 800 px' : '1200 × 500 px';
  const ratio = variant === 'desktop' ? 'aspect-[4/5]' : 'aspect-[12/5]';
  ...
  return (
    <div>
      ...
      {url ? (
        <div className={`relative overflow-hidden rounded-xl border bg-slate-50 ${ratio}`}>
          <Image
            src={url}
            alt={label}
            fill
        ...
        <button
          ...
          onClick={() => inputRef.current?.click()}
          disabled={uploading}
          className={`flex w-full flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 hover:border-green-300 hover:bg-green-50 ${ratio}`}
        >
          {uploading ? (
          ...
          <span className="mt-2 text-xs text-slate-500">
            {uploading ? 'Subiendo...' : 'Adjuntar imagen · máx. 5 MB'}
          </span>
        </button>
      )}
