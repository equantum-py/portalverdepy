...
export function HomeHero({ content, previewMode }: HomeHeroProps) {
  ...
  const showInPreview = previewMode
    ...
    : true;
  const showContent = content.heroContentEnabled && showInPreview;
  const responsiveContentClass = previewMode
    ...
    : !content.heroContentEnabled
      ? 'hidden'
      : content.heroContentDesktop && content.heroContentMobile
        ? 'flex'
        : content.heroContentDesktop
          ? 'hidden sm:flex'
          : content.heroContentMobile
  ...
  const horizontalClass =
    content.heroAlignment === 'center'
      ...
      : content.heroAlignment === 'right'
        ...
        : 'justify-start';
  const hasActions =
    content.heroShowPrimaryButton || content.heroShowSecondaryButton;
  ...
  return (
    <section
      aria-labelledby={content.heroShowTitle ? 'home-hero-title' : undefined}
      aria-label={content.heroShowTitle ? undefined : 'Portada principal'}
      className={`relative isolate overflow-hidden rounded-3xl bg-brand-950 shadow-soft ${
        previewMode === 'mobile' ? 'min-h-[460px]' : 'min-h-[430px] sm:min-h-[480px] lg:min-h-[470px]'
      }`}
    >
      {previewMode ? (
        previewImage ? (
          <Image src={previewImage} alt={content.heroAlt} fill sizes="900px" className="object-cover" />
        ) : null
      ) : (
        <>
          {content.heroDesktopUrl ? (
            <Image
              ...
              className={content.heroMobileUrl ? 'hidden object-cover sm:block' : 'object-cover'}
            />
          ) : null}
          {content.heroMobileUrl ? (
            <Image
              ...
              className={content.heroDesktopUrl ? 'object-cover sm:hidden' : 'object-cover'}
            />
          ) : null}
        </>
      )}
      ...
      {content.heroOverlay && showContent ? (
        <div
          ...
          style={{ opacity: content.heroOverlayIntensity / 100 }}
        />
      ) : null}
      ...
      >
        <div className={`flex max-w-xl flex-col gap-4 ${alignmentClass}`}>
          ...
          {content.heroShowTitle ? (
            <h1
              id="home-hero-title"
              className={`max-w-lg font-semibold leading-[1.08] tracking-tight text-white ${isPreviewMobile ? 'text-3xl' : 'text-3xl sm:text-4xl lg:text-5xl'}`}
            >
              {content.heroTitle}
            </h1>
          ) : null}
          ...
          {content.heroShowPrice ? (
            <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
              <span className="text-sm font-medium text-white/75">Desde</span>
          ...
          {hasActions ? (
            <div className={isPreviewMobile ? 'grid w-full gap-2.5' : 'grid w-full gap-2.5 sm:flex sm:w-auto sm:flex-wrap'}>
