...
import type { HeroCarouselSettings, HeroSlide } from "@/lib/home-content/hero-schema";
...
export function HomeHeroCarousel({ slides, settings, previewViewport }: Props) {
  const activeSlides = useMemo(
    () => slides.filter((s) => s.isActive).sort((a, b) => a.sortOrder - b.sortOrder),
    [slides],
  );
  ...
  const move = useCallback((direction: number) => {
    setIndex((current) => {
      ...
      if (settings.carouselLoop) return (next + activeSlides.length) % activeSlides.length;
  ...
  }, [activeSlides.length, move, paused, settings.carouselAutoplay, settings.carouselInterval]);
  ...
  return (
    ...
    >
      ...
      {controls && settings.carouselShowArrows && (
        <>
          <button type="button" aria-label="Diapositiva anterior" onClick={() => move(-1)} className="absolute left-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-black/40 p-2 text-white"><ChevronLeft /></button>
          <button type="button" aria-label="Diapositiva siguiente" onClick={() => move(1)} className="absolute right-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-black/40 p-2 text-white"><ChevronRight /></button>
        </>
      )}
      {controls && settings.carouselShowDots && (
        <div className="absolute bottom-2 left-1/2 z-20 flex -translate-x-1/2 gap-2">
          {activeSlides.map((slide, i) => (
            <button key={slide.id} type="button" aria-label={`Ir a diapositiva ${i + 1}`} aria-current={i === index} onClick={() => setIndex(i)} className={`h-2.5 w-2.5 rounded-full ring-1 ring-white ${i === index ? "bg-white" : "bg-white/40"}`} />
          ))}
        </div>
      )}
    </section>
